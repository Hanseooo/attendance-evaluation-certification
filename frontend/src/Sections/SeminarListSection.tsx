...
import FiltersPopover, { type Filters, } from "@/components/popover/FiltersPopover";
...
export default function SeminarListSection({ seminars, attendingIds = [], onAttend, className = "" }: Props) {
  ...
  return (
    <section className={`w-full ${className}`}>
      <div className="flex items-center justify-between mb-4 sticky top-0 z-10 bg-background/0">
        <div>
          <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Seminars</h3>
          <p className="text-sm text-muted-foreground mt-1">Browse upcoming seminars</p>
        </div>
      ...
      <div className="h-[60vh] md:h-[70vh] overflow-y-auto modal-scrollbar px-1">
        {processed.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-sm">No seminars match the current filters. Try resetting filters or toggling hide attending.</p>
          </div>
        ) : (
      ...
      <SeminarDetailsModal seminar={active ?? undefined} isOpen={open} onClose={closeDetails} onAttend={onAttend ?? (() => {})} />
