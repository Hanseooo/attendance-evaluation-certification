...
export function SeminarDetailsModal({ seminar, isOpen, onClose,  }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[85vh] bg-background border rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 bg-background/95 backdrop-blur-sm border-t sticky bottom-0">
          <div className="flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
            ...
            <Button onClick={handleAttend} className="w-full sm:w-auto">
              Attend Seminar
            </Button>
          </div>
        </div>
      ...
      <style>{`
        .detail-scrollbar::-webkit-scrollbar { width: 8px; }
        .detail-scrollbar::-webkit-scrollbar-track { background: hsl(var(--muted) / 0.3); border-radius: 4px; }
        .detail-scrollbar::-webkit-scrollbar-thumb { background: hsl(var(--primary) / 0.4); border-radius: 4px; transition: background 0.2s; }
        .detail-scrollbar::-webkit-scrollbar-thumb:hover { background: hsl(var(--primary) / 0.6); }
        .detail-scrollbar { scrollbar-width: thin; scrollbar-color: hsl(var(--primary) / 0.4) hsl(var(--muted) / 0.3); }
      `}</style>
    </div>
  );
}