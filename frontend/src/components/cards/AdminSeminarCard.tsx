...
export default function AdminSeminarCard({ seminar, onEdit, onDelete }: AdminSeminarCardProps) {
  return (
    <Card className="border border-border/50 shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all">
      <CardHeader className="flex flex-row items-start justify-between pb-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-32 bg-background">
            <DropdownMenuItem onClick={onEdit} className="flex items-center gap-2 hover:cursor-pointer hover:bg-black/5">
      ...
      <CardContent className="space-y-2 text-sm">
        <p className="text-muted-foreground line-clamp-2">{seminar.description}</p>
        <div className="pt-2 text-xs text-muted-foreground">
          <p>{new Date(seminar.date_start).toLocaleDateString()}</p>
          <p>
            Duration: {seminar.duration_minutes} mins
          </p>
          <p>{seminar.venue}</p>
        </div>
      </CardContent>
